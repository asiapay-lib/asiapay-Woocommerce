	...
	class WC_Bimopay extends WC_Payment_Gateway{
		...
		function init_form_fields(){
			...
			$this -> form_fields = array(
			  ...
			  'type' => 'text',
			  'description' => __('
			  	This is the payment URL of Bimopay (via client post through browser).')),
				'merchant_id' => array(
		...
		public function admin_options(){
			echo '<h3>'.__('Bimopay/Bimopay Payment Gateway').'</h3>';
			echo '<p>'.__('
		...
		public function generate_Bimopay_form($order_id){
			...
			if($this -> secure_hash_secret != ''){
				$secureHash = $this -> generatePaymentSecureHash($this -> merchant_id, $orderRef, $this -> curr_code, $order -> get_total(), $this -> pay_type, $this -> secure_hash_secret);
			}
		...
		function gateway_response(){
			...
			$eci = $_REQUEST['eci'];
			$payerAuth = $_REQUEST['payerAuth'];
			$sourceIp = $_REQUEST['sourceIp'];
			$ipCountry = $_REQUEST['ipCountry'];
			$secureHash = isset($_REQUEST['secureHash']) ? $_REQUEST['secureHash'] : "" ;
			
			echo "OK!";
			
			if( isset($_REQUEST['Ref']) && isset($_REQUEST['successcode']) && isset($_REQUEST['src']) && isset($_REQUEST['src']) ){
